...
class Bookings(models.Model):
    ...
    #state = fields.Selection(([('draft', 'Available'), ('converted', 'Converted')]), string='Status', default='draft')
    state = fields.Selection(([('draft', 'Available'), ('assigned', 'Assigned'),('cancel', 'Cancel')]), string='Status', 
                             group_expand='_read_group_stage_ids', default='draft')
    transport = fields.Selection(([('air', 'Air'), ('ocean', 'Ocean'), ('land', 'Land')]), default="ocean", string='Transport')
    ...
    shipment_id = fields.Many2one(string="Order AP#", 
        comodel_name='freight.operation')

    @api.model
    def _read_group_stage_ids(self, stages, domain, order):
        # stage_ids = self.env['shipment.stage'].search([])
        stage_ids =  [key for key, val in type(self).state.selection]
        return stage_ids
