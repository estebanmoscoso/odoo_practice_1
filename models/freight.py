...
class FreightOperation(models.Model):
    ...
    operation = fields.Selection([('direct', 'Direct'), ('house', 'House'), ('master', 'Master'), ('batch', 'Lote')], string='Operation')
    ...
    declaration_number = fields.Char('AES No.')
    ...
    import_regime = fields.Many2one('freight.import.regime', 'Régimen de Importación')
    audit_type = fields.Many2one('freight.audit.type', 'Tipo de Aforo')
    purchase_orders = fields.One2many('purchase.order', 'operation')
    cost_apportionment = fields.Many2one('stock.landed.cost', 'Prorrateo de Costos')
    invoices = fields.One2many('account.move', 'operation')
    stock_picking_receipts = fields.One2many('stock.picking', 'operation')
    ...
    def create(self, values):
        if not values.get('name', False) or values['name'] == _('New'):
            ...
            elif values.get('operation') == 'batch':
                values['name'] = self.env['ir.sequence'].next_by_code('operation.batch') or _('New')
        ...
        if id.booking_id:
            id.booking_id.write({'state': 'assigned','shipment_id':id.id})
...
class FreightImportRegime(models.Model):
    _name = 'freight.import.regime'
    _description = 'Régimen de Importación'
...
class FreightAuditType(models.Model):
    _name = 'freight.audit.type'
    _description = 'Tipos de Aforo'

    active = fields.Boolean(string='Activo')
    name = fields.Char(string='Nombre')
    type = fields.Selection(([
        ('documented', 'Documental'),
        ('physical', 'Físico'),
        ('electronic', 'Automático Electrónico'),
        ('not_intrusive', 'Automático no Intrusivo')
    ]), string='Tipo')
